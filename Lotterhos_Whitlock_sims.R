...
###################
   ...
   Filenames.env <- list.files(path="./dryad/SimFilesLFMM/", pattern="env")
   
   Env <- list()
   for(i in 1:length(Filenames.env))
   {
     tmp <- read.table(paste("./dryad/SimFilesLFMM/", Filenames.env[[i]], sep=""))
     Env[[i]] <- unlist(tmp)
   }
...
######################################################################   
   ...
   Filenames.lfmm <- list.files(path="./dryad/SimFilesLFMM/", pattern="lfmm")
...
# For normal applications, nPerm=199 should be sufficient for alpha = 0.05
   ...
   # -----------------------------
   TPR <- TP <- FPR <- TP.empirical <- TPR.empirical <- NumLoci <- TPR.msr <-
     FPR.msr <- TPR.both <- FPR.both <- TPR.empirical.both <- TPR.empirical.msr <- 
     Skewness <- Kurtosis <- Env.R2 <- MC <- S.neutral.sd <- Trend.max.neutral <- 
     Trend.max.selected <- rep(NA, nrow(Design))
   TPR <- FPR <- TP <- list(TPR, TPR, TPR)
   P.values <- P.values.both <- P.empirical.both <- list()
   ...
   for(j in 1:length(Sites.90))
   {
     ...
     tmp <- read.table(paste("./dryad/SimFilesLFMM/", Filenames.lfmm[[i]], sep=""))
     NumLoci[i] <- ncol(tmp)   
