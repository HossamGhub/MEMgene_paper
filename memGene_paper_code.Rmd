...
for(d in 1:10)
{
  Results.drop[[d]] <- mclapply(Index.j, function(j) getMEMgene(j, Dgen = Dgen.R.90, subset=NULL, drop=d),
                    mc.cores=detectCores())
}
  ...
  for(k in 1: length(Dgen[[j]]$Dgen))
  {
    ...
    if(length(drop) > 0)
    {
      Res[[k]] <- list(RsqAdjPos=RsqAdjPos, Morans.I=Morans.I, C=C, Rho=Rho, Range=Range,
                       ...
                       C.drop=C.drop, Rho.drop=Rho.drop, Range.drop=Range.drop, S.tot=S.tot, 
                       S.tot.drop=S.tot.drop)
    }
  }
  names(Res) <- names(Dgen[[1]]$Dgen)
