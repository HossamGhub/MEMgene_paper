...
with(Results.subsets.table[b,], boxplot(Fst.90 ~ Demography,
     main="Fst.90 (9900 loci)", ylim=c(0.011,0.015)))

with(Results.subsets.table[b,], boxplot(Fst.RsqAdj.90 ~ Demography,
     main="R^2.Adj.90 (9900 loci)", ylim=c(0.0,0.7)))

with(Results.subsets.table[b,], boxplot(Fst.Morans.I.90 ~ Demography,
     main="Morans.I.90 (9900 loci)", ylim=c(0.0,0.7)))

```

### b) With n=90 and removing IM: Interaction between #Ind (20, 6) and # Loci (9900, 3300, 500) in response variabe (Fst, adjusted R^2,  Moran's I, and Scaled MI).
...
par(mfcol=c(1,4))
with(Results.sub.long[noIM,], {interaction.plot(NumInd, NumLoci, Fst, col=c("red","green","blue"), main = "Fst")})
with(Results.sub.long[noIM,], {interaction.plot(NumInd, NumLoci, Adj.Rsqr.Pos, col=c("red","green","blue"), main = "Adjusted R^2 Positive")})
with(Results.sub.long[noIM,], {interaction.plot(NumInd, NumLoci, Morans.I, col=c("red","green","blue"), main = "Moran's I")})
with(Results.sub.long[noIM,], {interaction.plot(NumInd, NumLoci, Scaled.Morans.I, col=c("red","green","blue"), main = "Scaled Moran's I")})
...
with(Results.drop.MEM.table %>% filter(Demography == "1R" & NumInd == 20), 
     boxplot(data.frame(Drop.MEM=Fst.RsqAdjPos.drop.MEM.m / Fst.RsqAdjPos.90,
                        Drop=Fst.RsqAdjPos.drop.m / Fst.RsqAdjPos.90,
                        Drop.MEM.30=Fst.RsqAdjPos.30.m / Fst.RsqAdjPos.90),
             ylab="AdjRsqPos", main="1R", ylim=c(0.5,1)))
lines(c(0,10), c(1,1), lty=2)
...
with(Results.drop.MEM.table %>% filter(Demography == "1R" & NumInd == 20), 
     boxplot(data.frame(Drop.MEM=Fst.Morans.I.drop.MEM.m / Fst.Morans.I.90,
                        Drop=Fst.Morans.I.drop.m / Fst.Morans.I.90,
                        Drop.MEM.30=Fst.Morans.I.30.m / Fst.Morans.I.90),
             ylab="Morans.I", ylim=c(0.5,1)))
lines(c(0,10), c(1,1), lty=2)
...
with(Results.drop.MEM.table %>% filter(Demography == "1R" & NumInd == 20), 
     boxplot(data.frame(Drop.MEM=Fst.I.scaled.drop.MEM.m / Fst.I.scaled.90,
                        Drop=Fst.I.scaled.drop.m / Fst.I.scaled.90,
                        Drop.MEM.30=Fst.I.scaled.30.m / Fst.I.scaled.90),
             ylab="Scaled.Morans.I", ylim=c(0.5,1)))
lines(c(0,10), c(1,1), lty=2)
