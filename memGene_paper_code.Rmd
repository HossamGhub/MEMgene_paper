...
{
  cat(r)
  Results.drop5[[r]] <- mclapply(Index.j, function(j) getMEMgene(j, Dgen = Dgen.R.90,
                                                       subset=NULL, drop=Drop[[r]]),
                                mc.cores=detectCores())
  names(Results.drop5[[r]]) <- Sites.R.90
}
...
saveRDS(Results.drop5, paste0(here::here(), "/output/Results.drop5.rds"))
```
...
```{r}
Results.drop <- readRDS(paste0(here::here(), "/output/Results.drop5.rds"))
Results.subsets.table <-  readRDS(paste0(here::here(), "/output/Results.subsets.table.rds"))
```
...
for(k in 1:length(res.r[[1]]))
  {
    ...
    # RsqAdjPos.drop.MEM.m <- apply(sapply(res.r, function(sub) sub[[k]]$RsqAdjPos.drop), 1, mean)
    # RsqAdjPos.drop.MEM.s <- apply(sapply(res.r, function(sub) sub[[k]]$RsqAdjPos.drop), 1, sd)
    # Morans.I.drop.MEM.m <- apply(sapply(res.r, function(sub) sub[[k]]$Morans.I.drop), 1, mean)
    # Morans.I.drop.MEM.s <- apply(sapply(res.r, function(sub) sub[[k]]$Morans.I.drop), 1, sd)
    # C.drop.MEM.m <- apply(sapply(res.r, function(sub) sub[[k]]$C.drop), 1, mean)
    # C.drop.MEM.s <- apply(sapply(res.r, function(sub) sub[[k]]$C.drop), 1, sd)
    # Rho.drop.MEM.m <- apply(sapply(res.r, function(sub) sub[[k]]$Rho.drop), 1, mean)
    # Rho.drop.MEM.s <- apply(sapply(res.r, function(sub) sub[[k]]$Rho.drop), 1, sd)
    # I.scaled.drop.MEM.m <- apply(sapply(res.r, function(sub) sub[[k]]$I.scaled.drop), 1, mean)
    # I.scaled.drop.MEM.s <- apply(sapply(res.r, function(sub) sub[[k]]$I.scaled.drop), 1, sd)
    # I.scaled2.drop.MEM.m <- apply(sapply(res.r, function(sub) sub[[k]]$I.scaled2.drop), 1, mean)
    # I.scaled2.drop.MEM.s <- apply(sapply(res.r, function(sub) sub[[k]]$I.scaled2.drop), 1, sd)
    ...
    tmp[[k]] <- data.frame(S.tot.drop.m=S.tot.drop.m, S.tot.drop.s=S.tot.drop.s,
                           ...
                           I.scaled2.3drop.m=I.scaled2.drop.m, I.scaled2.drop.s=I.scaled2.drop.s)
                            
                           # RsqAdjPos.drop.MEM.m=RsqAdjPos.drop.MEM.m, RsqAdjPos.drop.MEM.s=RsqAdjPos.drop.MEM.s, 
                           # Morans.I.drop.MEM.m=Morans.I.drop.MEM.m, Morans.I.drop.MEM.s=Morans.I.drop.MEM.s,
                           # C.drop.MEM.m=C.drop.MEM.m, C.drop.MEM.s=C.drop.MEM.s, 
                           # Rho.drop.MEM.m=Rho.drop.MEM.m, Rho.drop.MEM.s=Rho.drop.MEM.s,
                           # I.scaled.drop.MEM.m=I.scaled.drop.MEM.m, I.scaled.drop.MEM.s=I.scaled.drop.MEM.s,
                           # I.scaled2.3drop.MEM.m=I.scaled2.drop.MEM.m, I.scaled2.drop.MEM.s=I.scaled2.drop.MEM.s
...
Results.drop5.table <- data.frame(Results.subsets.table, res.r.combined)
saveRDS(Results.drop5.table, paste0(here::here(), "/output/Results.drop5.table.rds"))
Results.drop5.table
```
...
```{r}
I.max.drop5 <- sapply(Results.drop, function(sub) max(sub[[1]][[1]]$Range))
I.min.drop5 <- sapply(Results.drop, function(sub) min(sub[[1]][[1]]$Range))
I.max.drop5.2 <- sapply(Results.drop, function(sub) max(sub[[1]][[1]]$Range.drop2))
I.min.drop5.2 <- sapply(Results.drop, function(sub) min(sub[[1]][[1]]$Range.drop1))
data.frame(I.max.drop5, I.min.drop5, I.max.drop5.2, I.min.drop5.2)
```
```{r}
mean(I.max.drop5.2)
mean(I.min.drop5.2)
...
```{r}
Results.drop5.table$Fst.RsqAdjPos.drop.m/Results.drop5.table$Fst.RsqAdjPos.90
Results.drop5.table$Fst.Morans.I.drop.m/Results.drop5.table$Fst.Morans.I.90
...
with(Results.drop5.table %>% filter(Demography == "1R"), 
     boxplot(data.frame(Drop=Fst.RsqAdjPos.drop.m, N90=Fst.RsqAdjPos.90),
             ylab="AdjRsqPos", main="1R"))

with(Results.drop5.table %>% filter(Demography == "2R"), 
     boxplot(data.frame(Drop=Fst.RsqAdjPos.drop.m, N90=Fst.RsqAdjPos.90),
             ylab="AdjRsqPos", main="2R"))

with(Results.drop5.table %>% filter(Demography == "IBD"), 
     boxplot(data.frame(Drop=Fst.RsqAdjPos.drop.m, N90=Fst.RsqAdjPos.90),
             ylab="AdjRsqPos", main="IBD"))

with(Results.drop5.table %>% filter(Demography == "1R"), 
     boxplot(data.frame(Drop=Fst.Morans.I.drop.m, N90=Fst.Morans.I.90),
             ylab="Morans.I"))
